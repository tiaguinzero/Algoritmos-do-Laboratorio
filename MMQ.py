...
u_delta_m = np.array([0.001, 0.001, 0.001, 0.001, 0.001]) # IMPORTANTE: Substitua pelas incertezas reais da massa

# Variável Dependente (Y): Aceleração linear (a) em m/s^2
a_exp = np.array([-1.09, -0.87, -0.67, -0.33, -0.99]) 
u_a_exp = np.array([0.04, 0.04, 0.04, 0.04, 0.04]) # IMPORTANTE: Substitua pelas incertezas reais da aceleração
...
I = (R**2) * ((g / A) - M) # Momento de Inércia(I)
tau_a = -B * ((g * R) / A) # Torque de Atrito
...
# PLOTAGEM DOS DADOS COM BARRAS DE INCERTEZA (Substituiu o plt.scatter)
plt.errorbar(delta_m, a_exp, xerr=u_delta_m, yerr=u_a_exp, fmt='s', color='red', 
             ecolor='black', elinewidth=1, capsize=3, capthick=1, ms=5, 
             label='Dados Experimentais', zorder=2)

# Desenha a reta do MMQ
